...
// Get COF department statistics
try {
    // Total COF employees (users with role 'user' AND teaching_status is not null)
    $total_result = $con->query("SELECT COUNT(*) as count FROM users WHERE department = 'COF' AND role = 'user' AND teaching_status IS NOT NULL AND teaching_status != ''");
    if ($total_result) {
    ...
    // Get unevaluated employees (COF users without any evaluation record)
    ...
    error_log("Database error in COF.php: " . $e->getMessage());
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_evaluation'])) {
    ...
    try {
        ...
        if ($stmt->affected_rows > 0) {
            ...
            header("Location: COF.php?success=1");
            exit();
        } else {
...
// Get real-time notifications for COF
$notifications = [];
try {
    // Recent evaluations for COF
    ...
    // Pending evaluations count for COF
    ...
    if ($pending_count) {
        ...
        if ($pending_row['count'] > 0) {
            $notifications[] = [
                'type' => 'pending',
                'message' => "You have {$pending_row['count']} pending evaluations in COF",
...
<head>
    ...
    <title>COF Admin Dashboard - LSPU</title>
    ...
    <style>
        ...
        /* Enhanced Sidebar Link Animations - BLUE THEME */
        .sidebar-link {
            transition: all 0.4s cubic-bezier(0.4, 0, 0.2, 1);
            border-radius: 15px;
            margin: 4px 0;
...
<body class="min-h-screen bg-white no-horizontal-scroll <?php echo $show_evaluation_modal ? 'overflow-hidden' : ''; ?>">
    <div class="flex h-screen">
        <!-- Sidebar - BLUE AND BLACK THEME -->
        <aside class="sidebar-container cof-gradient border-r border-blue-600">
            <div class="sidebar-content">
                ...
                <div class="navigation-section mb-3">
                    <nav class="mb-3">
                        <div class="space-y-1">
                            <a href="COF.php" class="flex items-center px-3 py-2 text-white font-semibold rounded-xl sidebar-link active sliding-line active">
                                <i class="ri-dashboard-line mr-2 text-base"></i>
                                <span class="text-sm">Dashboard</span>
                                <i class="ri-arrow-right-s-line ml-auto text-base"></i>
                            </a>
                            <a href="cof_eval.php" class="flex items-center px-3 py-2 text-white/90 font-semibold rounded-xl sidebar-link sliding-line">
        ...
        <div class="main-content">
            ...
            <header class="bg-gradient-to-r from-blue-800 to-blue-700 border-b border-blue-600">
                <div class="flex justify-between items-center px-6 py-4">
                    <div class="min-w-0">
                        <h1 class="text-2xl font-bold text-white truncate">Welcome back, <?= htmlspecialchars($user['name'] ?? 'COF Admin') ?>! 👋</h1>
                        <p class="text-white/70 text-sm mt-1 truncate">College of Fisheries - Faculty Evaluation Dashboard</p>
                    </div>
                        ...
                        <div class="p-4 card-content">
                            <div class="flex items-center justify-between">
                                <div class="min-w-0">
                                    ...
                                    <p class="text-white/70 text-xs mt-1 truncate">College of Fisheries</p>
                                </div>
                            ...
                            <?php if (!empty($unevaluated_employees)): ?>
                                    ...
                                    <?php foreach (array_slice($unevaluated_employees, 0, 5) as $employee): ?>
                                        <div class="evaluation-row sliding-line">
                                            ...
                                            <div class="evaluation-actions">
                                                <a href="cof_eval.php?user_id=<?= $employee['id'] ?>" 
                                                   class="btn-animated bg-gradient-to-r from-blue-500 to-blue-600 text-white px-4 py-2 rounded-lg text-xs font-bold hover:shadow-lg transition-all duration-300 transform hover:scale-105 shadow-md">
                                                    Evaluate Now
                                                </a>
                                            </div>
                                        </div>
                                ...
                                <?php if (count($unevaluated_employees) > 5): ?>
                                    <div class="mt-4 text-center">
                                        <a href="cof_eval.php" class="btn-animated text-blue-700 hover:text-blue-900 text-sm font-bold inline-flex items-center border-2 border-blue-700 px-4 py-2 rounded-lg hover:bg-blue-700 hover:text-white transition-all duration-300">
                                                ...
                                                <div class="flex gap-2">
                                                    ...
                                                    <form method="POST" action="" class="m-0">
                                                        ...
                                                        <button type="submit" name="view_evaluation" 
                                                                class="btn-animated bg-gradient-to-r from-blue-700 to-blue-800 text-white px-3 py-1.5 rounded text-xs font-bold hover:shadow-md transition-all duration-300">
                                                            View
                                                        </button>
                                                    </form>
                                                </div>
    ...
    <div id="notificationModal" class="fixed inset-0 z-50 hidden">
        ...
        <div class="modal-container bg-white rounded-2xl shadow-2xl absolute top-4 right-4 w-96 max-w-sm">
            ...
            <div class="p-4 border-t border-gray-200">
                <button class="w-full py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium" onclick="markAllAsRead()">
                    Mark all as read
                </button>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-2xl modal-container w-full max-w-6xl">
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="mt-8 flex justify-end space-x-4 pt-6 border-t">
                    ...
                    <a href="cof_eval.php?evaluation_id=<?= $evaluation_details['id'] ?>&user_id=<?= $evaluation_details['user_id'] ?>" class="btn-animated px-6 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors">
