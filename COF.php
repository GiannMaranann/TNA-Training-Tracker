...
try {
    ...
    $total_result = $con->query("SELECT COUNT(*) FROM users WHERE department = 'COF' AND role = 'user' AND teaching_status IS NOT NULL AND teaching_status != ''");
    $stats['total_employees'] = $total_result ? $total_result->fetch_row()[0] : 0;
    
    // Teaching staff
    $teaching_result = $con->query("SELECT COUNT(*) FROM users WHERE department = 'COF' AND teaching_status = 'Teaching' AND role = 'user'");
    $stats['teaching'] = $teaching_result ? $teaching_result->fetch_row()[0] : 0;
    
    // Non-teaching staff
    $non_teaching_result = $con->query("SELECT COUNT(*) FROM users WHERE department = 'COF' AND teaching_status = 'Non-Teaching' AND role = 'user'");
...
<head>
    ...
    <style>
        ...
        .card::before {
            ...
            right: 0;
            height: 100%;
            background: linear-gradient(135deg, rgba(8, 61, 119, 0.05) 0%, rgba(13, 102, 204, 0.1) 100%);
            opacity: 0;
        ...
        .card:hover {
            ...
            box-shadow: 0 25px 50px rgba(8, 61, 119, 0.25);
        }
        ...
        .sidebar-link.active {
            background: linear-gradient(135deg, #083d77 0%, #0d66cc 100%);
            box-shadow: 0 8px 25px rgba(8, 61, 119, 0.4);
        ...
        @keyframes pulseEnhanced {
            ...
            50% {
                ...
                box-shadow: 0 0 0 15px rgba(255, 107, 53, 0.3);
            }
            100% { 
                transform: scale(1); 
        ...
        .cof-gradient {
            background: linear-gradient(135deg, #083d77 0%, #0d66cc 50%, #1a75d2 100%);
        }
        ...
        .form-field:focus {
            border-color: #0d66cc;
            box-shadow: 0 0 0 3px rgba(13, 102, 204, 0.1);
            transform: translateY(-2px);
        }
        ...
        .text-campus-primary:hover {
            color: #0d66cc;
            text-shadow: 0 0 10px rgba(13, 102, 204, 0.3);
        }
        ...
        table tr:hover {
            background-color: rgba(8, 61, 119, 0.05) !important;
            transform: translateX(5px);
        }
    </style>
</head>
<body class="min-h-screen bg-white no-horizontal-scroll <?php echo isset($show_evaluation_modal) && $show_evaluation_modal ? 'overflow-hidden' : ''; ?>">
        ...
        <aside class="sidebar-container cof-gradient border-r border-blue-700">
            <div class="sidebar-content">
                ...
                <div class="navigation-section mb-3">
                    <nav class="mb-3">
                        <div class="space-y-1">
                            <a href="COF.php" class="flex items-center px-3 py-2 text-white font-semibold rounded-xl sidebar-link active">
                                <i class="ri-dashboard-line mr-2 text-base"></i>
                                <span class="text-sm">Dashboard</span>
                                <i class="ri-arrow-right-s-line ml-auto text-base"></i>
                            </a>
                            <a href="cof_eval.php" class="flex items-center px-3 py-2 text-white/90 font-semibold rounded-xl sidebar-link">
                            ...
                            <?php if (!empty($unevaluated_employees)): ?>
                                ...
                                <?php if (count($unevaluated_employees) > 5): ?>
                                    <div class="mt-4 text-center">
                                        <a href="cof_eval.php" class="btn-animated text-blue-900 hover:text-blue-700 text-sm font-bold inline-flex items-center border-2 border-blue-900 px-4 py-2 rounded-lg hover:bg-blue-900 hover:text-white transition-all duration-300">
