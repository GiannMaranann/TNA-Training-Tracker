...
// Get CBAA department statistics
try {
    // Total CBAA employees (users with role 'user' AND teaching_status is not null)
    $total_result = $con->query("SELECT COUNT(*) as count FROM users WHERE department = 'CBAA' AND role = 'user' AND teaching_status IS NOT NULL AND teaching_status != ''");
    if ($total_result) {
    ...
    error_log("Database error in CBAA.php: " . $e->getMessage());
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_evaluation'])) {
    ...
    try {
        ...
        if ($stmt->affected_rows > 0) {
            ...
            header("Location: CBAA.php?success=1");
            exit();
        } else {
...
<head>
    ...
    <title>CBAA Admin Dashboard - LSPU</title>
            ...
            <header class="bg-gradient-to-r from-yellow-800 to-yellow-700 border-b border-yellow-600">
                <div class="flex justify-between items-center px-6 py-4">
                    <div class="min-w-0">
                        ...
                        <p class="text-white/70 text-sm mt-1 truncate">College of Business, Administration and Accountancy - Faculty Evaluation Dashboard</p>
                    </div>
                        ...
                        <div class="p-4 card-content">
                            <div class="flex items-center justify-between">
                                <div class="min-w-0">
                                    ...
                                    <p class="text-white/70 text-xs mt-1 truncate">College of Business, Administration and Accountancy</p>
                                </div>
