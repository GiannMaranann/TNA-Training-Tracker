...
// Get CTE department statistics
try {
    // Total CTE employees (users with role 'user' AND teaching_status is not null)
    $total_result = $con->query("SELECT COUNT(*) as count FROM users WHERE department = 'CTE' AND role = 'user' AND teaching_status IS NOT NULL AND teaching_status != ''");
    if ($total_result) {
    ...
    $evaluation_stats_result = $con->query("
        ...
        FROM evaluations
        WHERE EXISTS (
            SELECT 1 FROM users u WHERE u.id = evaluations.user_id AND u.department = 'CTE'
        )
    ");
    ...
    // Get unevaluated faculty (CTE users without any evaluation record for current year)
    ...
    error_log("Database error in CTE.php: " . $e->getMessage());
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_evaluation'])) {
    ...
    try {
        ...
        if ($stmt->affected_rows > 0) {
            ...
            header("Location: CTE.php?success=1");
            exit();
        } else {
...
<head>
    ...
    <title>CTE Admin Dashboard - LSPU</title>
    ...
    <style>
        ...
        .card::before {
            ...
            right: 0;
            height: 100%;
            background: linear-gradient(135deg, rgba(123, 97, 255, 0.05) 0%, rgba(156, 136, 255, 0.1) 100%);
            opacity: 0;
        ...
        .sidebar-link:hover {
            background: linear-gradient(135deg, #7b61ff 0%, #9c88ff 100%);
        ...
        .stat-card:nth-child(2) {
            background: linear-gradient(135deg, #9c88ff 0%, #a594ff 100%);
        }
        
        .stat-card:nth-child(3) {
            background: linear-gradient(135deg, #a594ff 0%, #b19fff 100%);
        }
        
        .stat-card:nth-child(4) {
            background: linear-gradient(135deg, #b19fff 0%, #beabff 100%);
        }
        ...
        @keyframes pulseEnhanced {
            ...
            50% {
                ...
                box-shadow: 0 0 0 15px rgba(123, 97, 255, 0.3);
            }
            100% { 
                transform: scale(1); 
        ...
        .lspu-header {
            background: linear-gradient(135deg, 
                rgba(123, 97, 255, 0.95) 0%, 
                rgba(156, 136, 255, 0.95) 50%, 
                rgba(177, 159, 255, 0.95) 100%);
            backdrop-filter: blur(20px);
        ...
        /* CTE-specific styling */
        .cte-gradient {
            background: linear-gradient(135deg, #7b61ff 0%, #9c88ff 50%, #b19fff 100%);
        }
        ...
        .evaluation-item::before {
            ...
            width: 4px;
            height: 100%;
            background: linear-gradient(to bottom, #ff6b6b, #ffa8a8);
            transform: scaleY(0);
        ...
        .text-campus-primary:hover {
            color: #9c88ff;
            text-shadow: 0 0 10px rgba(156, 136, 255, 0.3);
        }
...
<body class="min-h-screen bg-white no-horizontal-scroll <?php echo $show_evaluation_modal ? 'overflow-hidden' : ''; ?>">
    <div class="flex h-screen">
        <!-- Sidebar -->
        <aside class="sidebar-container cte-gradient border-r border-purple-700">
            <div class="sidebar-content">
                ...
                <div class="lspu-header p-3 text-white mb-3 rounded-xl">
                    <div class="text-center relative z-10">
                        ...
                        <div class="flex items-center justify-center space-x-3 mb-3">
                            ...
                            <!-- CTE Logo -->
                            <div class="logo-container">
                                <img src="images/cte.png" alt="CTE Logo" class="w-12 h-12 rounded-lg bg-white p-1"
                                     onerror="this.style.display='none'; this.nextElementSibling.style.display='flex'">
                                <div class="w-12 h-12 bg-white/20 rounded-lg flex items-center justify-center backdrop-blur-sm" style="display: none;">
                        ...
                        <div class="border-t border-white/30 pt-2">
                            ...
                            <div class="mt-2 pt-2 border-t border-white/30">
                                <h3 class="text-sm font-bold uppercase tracking-wide">COLLEGE OF TEACHER EDUCATION</h3>
                                <p class="text-xs opacity-80 mt-1 font-semibold text-accent-coral">A.Y. 2024-2025</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="navigation-section mb-3">
                    <nav class="mb-3">
                        <div class="space-y-1">
                            <a href="CTE.php" class="flex items-center px-3 py-2 text-white font-semibold rounded-xl sidebar-link active">
                                <i class="ri-dashboard-line mr-2 text-base"></i>
                                <span class="text-sm">Dashboard</span>
                                <i class="ri-arrow-right-s-line ml-auto text-base"></i>
                            </a>
                            <a href="cte_eval.php" class="flex items-center px-3 py-2 text-white/90 font-semibold rounded-xl sidebar-link">
        ...
        <div class="main-content">
            <!-- Header -->
            <header class="bg-gradient-to-r from-purple-900 to-purple-800 border-b border-purple-700">
                <div class="flex justify-between items-center px-6 py-4">
                    <div class="min-w-0">
                        <h1 class="text-2xl font-bold text-white truncate">Welcome back, <?= htmlspecialchars($user['name'] ?? 'CTE Admin') ?>! 👋</h1>
                        <p class="text-white/70 text-sm mt-1 truncate">College of Teacher Education - Faculty Evaluation Dashboard</p>
                    </div>
                    <div class="flex items-center space-x-4 flex-shrink-0">
                        <!-- Notification Bell -->
                        <div class="relative">
                            <button class="notification-bell text-white hover:text-purple-200 transition-colors">
            ...
            <main class="p-6 bg-gray-50 min-h-full">
                        ...
                        <div class="p-4 card-content">
                            <div class="flex items-center justify-between">
                                <div class="min-w-0">
                                    ...
                                    <p class="text-white/70 text-xs mt-1 truncate">College of Teacher Education</p>
                                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                    ...
                    <div class="card">
                        <div class="p-4 card-content">
                            <h3 class="text-lg font-bold text-gray-800 mb-4 flex items-center">
                                <i class="ri-progress-4-line mr-2 text-purple-900 text-xl"></i>
                    ...
                    <div class="card">
                        <div class="p-4 card-content">
                            <h3 class="text-lg font-bold text-gray-800 mb-4 flex items-center">
                                <i class="ri-pie-chart-line mr-2 text-purple-900 text-xl"></i>
                                Faculty Distribution
                            </h3>
                            <div class="h-64">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <?php if (!empty($unevaluated_faculty)): ?>
                                <div class="space-y-3">
                                    <?php foreach (array_slice($unevaluated_faculty, 0, 5) as $faculty): ?>
                                        <div class="evaluation-item flex items-center justify-between p-3 bg-gradient-to-r from-yellow-50 to-orange-50 rounded-xl border-2 border-yellow-200 hover:border-yellow-400 transition-all duration-300">
                                            ...
                                            <a href="cte_eval.php?user_id=<?= $faculty['id'] ?>" 
                                ...
                                <?php if (count($unevaluated_faculty) > 5): ?>
                                    <div class="mt-4 text-center">
                                        <a href="cte_eval.php" class="btn-animated text-purple-900 hover:text-purple-700 text-sm font-bold inline-flex items-center border-2 border-purple-900 px-4 py-2 rounded-lg hover:bg-purple-900 hover:text-white transition-all duration-300">
                    ...
                    <div class="card">
                        <div class="p-4 card-content">
                            <div class="flex items-center justify-between mb-4">
                                <h3 class="text-lg font-bold text-gray-800 flex items-center">
                                    <i class="ri-history-line mr-2 text-purple-900 text-xl"></i>
                                    Recent Evaluations
                                </h3>
                                <span class="bg-purple-100 text-purple-800 text-xs font-bold px-2 py-1 rounded-full border border-purple-300 status-badge">
                                ...
                                <div class="space-y-3">
                                    <?php foreach ($recent_evaluations as $evaluation): 
                                        $status_colors = [
                                            ...
                                            'submitted' => ['bg' => 'from-purple-500 to-purple-600', 'text' => 'text-purple-800', 'badge' => 'bg-purple-100 border-purple-300'],
                                    ...
                                    ?>
                                        <div class="evaluation-item flex items-center justify-between p-3 bg-gradient-to-r from-gray-50 to-purple-50 rounded-xl border-2 border-purple-200 hover:border-purple-400 transition-all duration-300">
                                            ...
                                            <div class="flex items-center space-x-2 flex-shrink-0 ml-2">
                                                ...
                                                <form method="POST" action="" class="m-0">
                                                    ...
                                                    <button type="submit" name="view_evaluation" 
                                                            class="btn-animated bg-gradient-to-r from-purple-900 to-purple-800 text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:shadow-md transition-all duration-300 shadow-sm">
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 modal-backdrop">
        <div class="bg-white rounded-2xl shadow-2xl modal-container w-full max-w-6xl">
            <!-- Modal Header -->
            <div class="bg-gradient-to-r from-purple-900 to-purple-800 text-white p-6 rounded-t-2xl">
                <div class="flex items-center justify-between">
                    <div>
            ...
            <div class="p-6 space-y-6">
                                ...
                                <div class="flex justify-between items-start">
                                    <div>
                                        <p class="font-medium text-gray-800">
                                            Status changed from 
                                            <span class="text-purple-600"><?= $history['from_status'] ? ucfirst($history['from_status']) : 'None' ?></span> 
                                            to 
                ...
                <div class="mt-8 flex justify-end space-x-4 pt-6 border-t">
                    ...
                    <a href="cte_eval.php?evaluation_id=<?= $evaluation_details['id'] ?>&user_id=<?= $evaluation_details['user_id'] ?>" class="btn-animated px-6 py-2 bg-purple-600 text-white rounded hover:bg-purple-700 transition-colors">
