...
// Get CA department statistics
try {
    // Total CA employees (users with role 'user' AND teaching_status is not null)
    $total_result = $con->query("SELECT COUNT(*) as count FROM users WHERE department = 'CA' AND role = 'user' AND teaching_status IS NOT NULL AND teaching_status != ''");
    if ($total_result) {
    ...
    $evaluation_stats_result = $con->query("
        ...
        FROM evaluations
        WHERE EXISTS (
            SELECT 1 FROM users u WHERE u.id = evaluations.user_id AND u.department = 'CA'
        )
    ");
    ...
    // Get unevaluated faculty (CA users without any evaluation record for current year)
    ...
    error_log("Database error in CA.php: " . $e->getMessage());
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_evaluation'])) {
    ...
    try {
        ...
        if ($stmt->affected_rows > 0) {
            ...
            header("Location: CA.php?success=1");
            exit();
        } else {
...
<head>
    ...
    <title>CA Admin Dashboard - LSPU</title>
    ...
    <style>
        ...
        /* Enhanced Sidebar Link Animations - GREEN AND BROWN THEME */
        .sidebar-link {
            transition: all 0.4s cubic-bezier(0.4, 0, 0.2, 1);
            border-radius: 15px;
            margin: 4px 0;
...
<body class="min-h-screen bg-white no-horizontal-scroll <?php echo $show_evaluation_modal ? 'overflow-hidden' : ''; ?>">
    <div class="flex h-screen">
        <!-- Sidebar - GREEN AND BROWN THEME -->
        <aside class="sidebar-container ca-gradient border-r border-green-600">
            <div class="sidebar-content">
                ...
                <div class="lspu-header p-3 text-white mb-3 rounded-xl">
                    <div class="text-center relative z-10">
                        ...
                        <div class="flex items-center justify-center space-x-3 mb-3">
                            ...
                            <!-- CA Logo -->
                            <div class="logo-container">
                                <img src="images/ca.png" alt="CA Logo" class="w-12 h-12 rounded-lg bg-white p-1"
                                     onerror="this.style.display='none'; this.nextElementSibling.style.display='flex'">
                                <div class="w-12 h-12 bg-white/20 rounded-lg flex items-center justify-center backdrop-blur-sm" style="display: none;">
                                    <i class="ri-plant-line text-white text-lg"></i>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="border-t border-white/30 pt-2">
                            ...
                            <div class="mt-2 pt-2 border-t border-white/30">
                                <h3 class="text-sm font-bold uppercase tracking-wide">COLLEGE OF AGRICULTURE</h3>
                                <p class="text-xs opacity-80 mt-1 font-semibold text-accent-brown">A.Y. 2024-2025</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="navigation-section mb-3">
                    <nav class="mb-3">
                        <div class="space-y-1">
                            <a href="CA.php" class="flex items-center px-3 py-2 text-white font-semibold rounded-xl sidebar-link active sliding-line active">
                                <i class="ri-dashboard-line mr-2 text-base"></i>
                                <span class="text-sm">Dashboard</span>
                                <i class="ri-arrow-right-s-line ml-auto text-base"></i>
                            </a>
                            <a href="ca_eval.php" class="flex items-center px-3 py-2 text-white/90 font-semibold rounded-xl sidebar-link sliding-line">
        ...
        <div class="main-content">
            ...
            <header class="bg-gradient-to-r from-green-800 to-green-700 border-b border-green-600">
                <div class="flex justify-between items-center px-6 py-4">
                    <div class="min-w-0">
                        <h1 class="text-2xl font-bold text-white truncate">Welcome back, <?= htmlspecialchars($user['name'] ?? 'CA Admin') ?>! 👋</h1>
                        <p class="text-white/70 text-sm mt-1 truncate">College of Agriculture - Faculty Evaluation Dashboard</p>
                    </div>
                        ...
                        <div class="p-4 card-content">
                            <div class="flex items-center justify-between">
                                <div class="min-w-0">
                                    ...
                                    <p class="text-white/70 text-xs mt-1 truncate">College of Agriculture</p>
                                </div>
                            ...
                            <?php if (!empty($unevaluated_faculty)): ?>
                                <div class="space-y-3">
                                    <?php foreach (array_slice($unevaluated_faculty, 0, 5) as $faculty): ?>
                                        <div class="evaluation-item flex items-center justify-between p-3 bg-gradient-to-r from-yellow-50 to-orange-50 rounded-xl border-2 border-yellow-200 hover:border-yellow-400 transition-all duration-300 sliding-line">
                                            ...
                                            <a href="ca_eval.php?user_id=<?= $faculty['id'] ?>" 
                                ...
                                <?php if (count($unevaluated_faculty) > 5): ?>
                                    <div class="mt-4 text-center">
                                        <a href="ca_eval.php" class="btn-animated text-green-700 hover:text-green-900 text-sm font-bold inline-flex items-center border-2 border-green-700 px-4 py-2 rounded-lg hover:bg-green-700 hover:text-white transition-all duration-300">
        ...
        <div class="bg-white rounded-2xl shadow-2xl modal-container w-full max-w-6xl">
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="mt-8 flex justify-end space-x-4 pt-6 border-t">
                    ...
                    <a href="ca_eval.php?evaluation_id=<?= $evaluation_details['id'] ?>&user_id=<?= $evaluation_details['user_id'] ?>" class="btn-animated px-6 py-2 bg-green-600 text-white rounded hover:bg-green-700 transition-colors">
