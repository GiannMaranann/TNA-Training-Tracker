...
// Get CONAH department statistics
try {
    // Total CONAH employees (users with role 'user' AND teaching_status is not null)
    $total_result = $con->query("SELECT COUNT(*) as count FROM users WHERE department = 'CONAH' AND role = 'user' AND teaching_status IS NOT NULL AND teaching_status != ''");
    if ($total_result) {
    ...
    $evaluation_stats_result = $con->query("
        ...
        FROM evaluations
        WHERE EXISTS (
            SELECT 1 FROM users u WHERE u.id = evaluations.user_id AND u.department = 'CONAH'
        )
    ");
    ...
    // Get unevaluated faculty (CONAH users without any evaluation record for current year)
    ...
    error_log("Database error in CONAH.php: " . $e->getMessage());
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_evaluation'])) {
    ...
    try {
        ...
        if ($stmt->affected_rows > 0) {
            ...
            header("Location: CONAH.php?success=1");
            exit();
        } else {
...
<head>
    ...
    <title>CONAH Admin Dashboard - LSPU</title>
...
<body class="min-h-screen bg-white no-horizontal-scroll <?php echo $show_evaluation_modal ? 'overflow-hidden' : ''; ?>">
        ...
        <aside class="sidebar-container conah-gradient border-r border-gray-800">
            <div class="sidebar-content">
                ...
                <div class="navigation-section mb-3">
                    <nav class="mb-3">
                        <div class="space-y-1">
                            <a href="CONAH.php" class="flex items-center px-3 py-2 text-white font-semibold rounded-xl sidebar-link active sliding-line active">
                                <i class="ri-dashboard-line mr-2 text-base"></i>
                                <span class="text-sm">Dashboard</span>
                                <i class="ri-arrow-right-s-line ml-auto text-base"></i>
                            </a>
                            <a href="conah_eval.php" class="flex items-center px-3 py-2 text-white/90 font-semibold rounded-xl sidebar-link sliding-line">
            ...
            <header class="bg-gradient-to-r from-gray-900 to-gray-800 border-b border-gray-700">
                <div class="flex justify-between items-center px-6 py-4">
                    <div class="min-w-0">
                        ...
                        <p class="text-white/70 text-sm mt-1 truncate">College of Nursing and Allied Health - Faculty Evaluation Dashboard</p>
                    </div>
                        ...
                        <div class="p-4 card-content">
                            <div class="flex items-center justify-between">
                                <div class="min-w-0">
                                    ...
                                    <p class="text-white/70 text-xs mt-1 truncate">College of Nursing and Allied Health</p>
                                </div>
                            ...
                            <?php if (!empty($unevaluated_faculty)): ?>
                                <div class="space-y-3">
                                    <?php foreach (array_slice($unevaluated_faculty, 0, 5) as $faculty): ?>
                                        <div class="evaluation-item flex items-center justify-between p-3 bg-gradient-to-r from-yellow-50 to-green-50 rounded-xl border-2 border-yellow-200 hover:border-yellow-400 transition-all duration-300 sliding-line">
                                            ...
                                            <a href="conah_eval.php?user_id=<?= $faculty['id'] ?>" 
                                ...
                                <?php if (count($unevaluated_faculty) > 5): ?>
                                    <div class="mt-4 text-center">
                                        <a href="conah_eval.php" class="btn-animated text-gray-900 hover:text-gray-700 text-sm font-bold inline-flex items-center border-2 border-gray-900 px-4 py-2 rounded-lg hover:bg-gray-900 hover:text-white transition-all duration-300">
                        ...
                        <div class="p-4 card-content">
                            ...
                            <?php if (!empty($recent_evaluations)): ?>
                                <div class="space-y-3">
                                    ...
                                    ?>
                                        <div class="evaluation-item flex items-center justify-between p-3 bg-gradient-to-r from-gray-50 to-green-50 rounded-xl border-2 border-gray-200 hover:border-gray-400 transition-all duration-300 sliding-line">
        ...
        <div class="bg-white rounded-2xl shadow-2xl modal-container w-full max-w-6xl">
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="mt-8 flex justify-end space-x-4 pt-6 border-t">
                    ...
                    <a href="conah_eval.php?evaluation_id=<?= $evaluation_details['id'] ?>&user_id=<?= $evaluation_details['user_id'] ?>" class="btn-animated px-6 py-2 bg-gray-900 text-white rounded hover:bg-gray-800 transition-colors">
    ...
    <script>
        ...
        function closeModal() {
            ...
            window.location.href = 'CONAH.php';
        }
